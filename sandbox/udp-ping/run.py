...
def executable_paths(implementation: str) -> tuple[Path, Path]:
    root = Path("sandbox/udp-ping") / implementation
    target_prefix = f"sandbox_udp_ping_{implementation}"
    return (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="UDP ping implementation to run.",
    )
    parser.add_argument(
        "--build-dir",
        type=Path,
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Build them first with: make udp-ping-{args.implementation}",
            file=sys.stderr,
        )
        return 1
